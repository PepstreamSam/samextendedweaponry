...
{
	public class bombastic : ModItem
	{
		public override void SetStaticDefaults() 
		{
			DisplayName.SetDefault("Mr Bombastic"); // By default, capitalization in classnames will add spaces to the display name. You can customize the display
			Tooltip.SetDefault("Something Fantastic.");
		}
		...
		{
			Projectile.NewProjectile(position.X, position.Y, speedX, speedY, ProjectileID.HellfireArrow, damage, knockBack, player.whoAmI);
			...
			type = Main.rand.Next(new int[] { type, ProjectileID.GrenadeI, ProjectileID.VortexBeaterRocket, });
			type = Main.rand.Next(new int[] { type, ProjectileID.MolotovCocktail, ProjectileID.Spark, });
			type = Main.rand.Next(new int[] { type, ProjectileID.RocketFireworksBoxBlue, ProjectileID.BlackBolt, });
		...
		public override void AddRecipes() {
			...
			recipe.AddIngredient(ItemID.HellstoneBar, 73);
			recipe.AddIngredient(ItemID.Hellstone, 100);
			recipe.AddIngredient(ItemID.Fireblossom, 25);
			recipe.AddIngredient(ItemID.ExplodingBullet, 1000);
