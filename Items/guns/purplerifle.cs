...
{
	public class purplerifle : ModItem
	{
		public override void SetStaticDefaults() 
		{
			DisplayName.SetDefault("Three Barrel'd Musket"); // By default, capitalization in classnames will add spaces to the display name. You can customize the display
			Tooltip.SetDefault("Shoots a burst of bullets and a laser cursed bullets and explosive bullets(all for one and one for all).");
		}
		...
		public override bool Shoot(Player player, ref Vector2 position, ref float speedX, ref float speedY, ref int type, ref int damage, ref float knockBack)
		{
			...
			// By returning true, the vanilla behavior will take place, which will shoot the 1st projectile, the one determined by the ammo.
			{
				...
				type = Main.rand.Next(new int[] { type, ProjectileID.TinyEater, ProjectileID.CursedBullet });
				return true;
			}
		}
